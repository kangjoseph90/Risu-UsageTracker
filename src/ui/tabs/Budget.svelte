<script lang="ts">
    ...
    $: filteredRulesWithUsage = searchQuery.trim()
        ...
        : rulesWithUsage;
    $: sortedRules = [...filteredRulesWithUsage].sort(
...
<div class="flex flex-col h-full">
    ...
    <div class="flex-shrink-0 px-3 pt-3 pb-2">
        <div
            class="rounded-lg bg-zinc-800 border border-zinc-700/60 hover:border-zinc-600/60 transition-colors overflow-hidden"
        >
            ...
            >
                ...
                >
                    <div
                        class="text-zinc-500 group-hover:text-zinc-400"
                        class:rotate-180={isAddRuleExpanded}
                    >
                        <ChevronDown size={16} />
                    </div>
                    <h3
                        class="text-sm font-semibold text-zinc-200 group-hover:text-zinc-100 transition-colors"
                    >
                        {$language.addRule}
                    </h3>
                </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <select
                                ...
                                class="bg-zinc-700 text-zinc-200 border border-zinc-700/60 rounded px-2 py-1 text-xs w-full"
                            >
                                <option value={BudgetPeriod.Daily}
                                    >{$language.daily}</option
                                >
                                <option value={BudgetPeriod.Weekly}
                                    >{$language.weekly}</option
                                >
                                <option value={BudgetPeriod.Monthly}
                                    >{$language.monthly}</option
                                >
                            </select>
                        </div>
                        ...
                        <tr class="hover:bg-zinc-800/50 transition-colors">
                            ...
                            >
                                {#if editingRuleId === rule.id}
                                    ...
                                    />
                                {:else}
                                    <div class="truncate" title={rule.name}>
                                        {rule.name}
                                    </div>
                                {/if}
                            </td>
                                ...
                                {:else}
                                    <div class="flex flex-col gap-1">
                                        <div class="flex items-center gap-2">
                                            <span
                                                class="text-xs text-zinc-500 min-w-fit"
                                                >{$language.model}:</span
                                            >
                                            {#if rule.model}
                                                <span
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            {#if rule.requestType}
                                                <span
                                                    class="text-xs text-zinc-500"
                                                    >{rule.requestType}</span
                                                >
                                            {:else}
                                                <span
                                                    class="text-xs text-zinc-500"
                                                    >{$language.allTypes}</span
                                                >
                                            {/if}
                                        </div>
                                    </div>
                            ...
                            <td class="px-4 py-2 text-sm whitespace-nowrap">
                                <div class="flex flex-col gap-1">
                                    ...
                                    />
                                    <div
                                        class="w-full bg-zinc-700 rounded-full h-1.5"
                                    >
                                        <div
                                            class="h-1.5 rounded-full transition-all"
                                            class:bg-green-500={percentage <=
                                                70}
                                            class:bg-yellow-500={percentage >
                                                70 && percentage <= 90}
                                            class:bg-orange-500={percentage >
                                                90 && percentage <= 100}
