...
<tr
    class="hover:bg-zinc-800/50 transition-colors {isSelected
        ? 'bg-zinc-800'
        : ''}"
>
    <td class="px-4 py-2">
    ...
    <td class="pr-4 py-2 text-sm">
        <div class="space-y-0.5">
            <div class="font-medium text-zinc-200 whitespace-nowrap">
                {record.model}
            </div>
            ...
            <div class="text-xs text-zinc-500 whitespace-nowrap">
                {record.requestType || RequestType.Unknown} • {new Date(
                    record.timestamp
                ).toLocaleString()}
            </div>
        </div>
        ...
        <div class="space-y-0.5">
            <div class="flex justify-between gap-2">
                ...
                <span class="text-zinc-300 text-xs"
                    >{(record.inputTokens ?? 0).toLocaleString()}</span
                >
            </div>
            {#if record.cachedInputTokens > 0}
                <div class="flex justify-between gap-2">
                    <span class="text-zinc-400 text-xs"
                        >{$language.cached}:</span
                    >
                    <span class="text-zinc-300 text-xs"
                        >{record.cachedInputTokens.toLocaleString()}</span
                    >
                </div>
            {/if}
            <div class="flex justify-between gap-2">
                ...
                <span class="text-zinc-300 text-xs"
                    >{(record.outputTokens ?? 0).toLocaleString()}</span
                >
            </div>
        </div>
    </td>
    <td class="px-4 py-2 text-sm whitespace-nowrap">
        <div class="space-y-0.5">
            <div class="flex justify-between gap-2">
                ...
                >
                <span class="text-zinc-300 text-xs">
                    <DollarDisplay amount={record.totalCost} />
                </span>
            </div>
            <div class="flex justify-between gap-2">
                <span class="text-zinc-400 text-xs">{$language.input}:</span>
                <span class="text-zinc-300 text-xs">
                    <DollarDisplay amount={record.inputCost} />
                </span>
            </div>
            <div class="flex justify-between gap-2">
                <span class="text-zinc-400 text-xs">{$language.output}:</span>
                <span class="text-zinc-300 text-xs">
                    <DollarDisplay amount={record.outputCost} />
                </span>
            </div>
        </div>
    </td>
