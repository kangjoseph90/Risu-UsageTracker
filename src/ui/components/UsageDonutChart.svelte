<script lang="ts">
    ...
    function handleSegmentInteraction(
        ...
        <div class="flex flex-col gap-2 flex-1 min-w-[200px]">
            {#each topData as item, index}
                <div class="flex items-center gap-3 text-sm">
                    ...
                    ></span>
                    <span
                        class="text-zinc-300 flex-1 overflow-hidden text-ellipsis whitespace-nowrap min-w-0"
                        title={item.name}
                    >
                        {item.name}
                    </span>
    ...
    {#if tooltipData}
        ...
        >
            ...
            <div class="space-y-0.5 text-zinc-300">
                <div>
                    {$language.tokens}: {formatNumber(tooltipData.tokens)}
                </div>
                <div class="flex items-center gap-1">
                    <span>{$language.cost}:</span>
                    <DollarDisplay
                        amount={tooltipData.cost}
